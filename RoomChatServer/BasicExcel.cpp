...
{
	wchar_t strUnicode[NameSize] = { 0, };
	char* strMultibyte = new char[NameSize];
	//char	strMultibyte[256] = { 0, };
	wcscpy_s(strUnicode, NameSize, cell->GetWString());
...
bool BasicExcel::ReadExcel(char * excelName)
{
	BasicExcel e;
	...
	if (sheet1)
	{
		...
		for (size_t r = 1; r<maxRows; ++r)
		{
			...
			for (size_t c = 0; c<maxCols; ++c)
			{
				...
				if (c == CharacterNameCols)
				{
					name = CharToMultiByte(cell);
					continue;
				}
				SelectSaveCols(c, cell, characterNumber, characterLVCols);
			}
			if (nullptr != name)
			{
				cout << "액셀 읽기 성공" << endl;
